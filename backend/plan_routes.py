...
) -> tuple[dict[str, object], dict[str, list[str]], list[dict], list[Element]]:
    import ifcopenshell

    model = ifcopenshell.open(str(ifc_path))
    plan_elements = [replace(element, extra=dict(element.extra)) for element in elements]
    footprints = _element_footprints(model, plan_elements)
    _set_ramp_display_storeys(plan_elements, _storey_elevations(model))
    open_boundaries = _open_boundary_portals(model, plan_elements, footprints)
    floor_refs = _floor_refs(plan_elements)
    spaces_by_door = _door_to_spaces(model)
    _add_geometric_door_spaces(spaces_by_door, plan_elements, footprints, floor_refs)
    _filter_door_spaces(spaces_by_door, plan_elements, footprints, _door_normals(model, plan_elements))
    return footprints, spaces_by_door, open_boundaries, plan_elements
...
def build_plan_route_edges(
    ...
    prepared: tuple[dict[str, object], dict[str, list[str]], list[dict], list[Element]] | None = None,
) -> list[RouteEdge]:
    footprints, spaces_by_door, open_boundaries, plan_elements = prepared or prepare_plan_geometry(ifc_path, elements)
    candidates = _plan_candidates(plan_elements, footprints, spaces_by_door, route_edges)
    candidates.extend(_open_boundary_candidates(plan_elements, footprints, spaces_by_door, open_boundaries, candidates))
    candidates.extend(_exterior_ramp_candidates(plan_elements, footprints, spaces_by_door))
    return build_plan_network(plan_elements, candidates, route_edges)
...
def _exterior_ramp_candidates(
    ...
    for ramp in elements:
        if ramp.ifc_type not in {"IfcRamp", "IfcRampFlight"}:
            continue
        lower = _ramp_point(ramp, "planRampLowerPoint")
        upper = _ramp_point(ramp, "planRampUpperPoint")
        ...
        measurements = _route_measurements(door, ramp, path, obstacles, None)
        ...
        for route_key, plan_key, ramp_key in [
            ("routeRampSlopePercent", "planRampSlopePercent", "rampSlopePercent"),
            ("routeRampUsableWidthM", "planRampUsableWidthM", "rampUsableWidthM"),
            ("routeRampRunLengthM", "planRampRunLengthM", "rampRunLengthM"),
        ]:
            value = _number(ramp.extra.get(plan_key))
            if value is None:
...
def _ramp_axis(ramp: Element) -> tuple[float, float] | None:
    x = _number(ramp.extra.get("planRampRunAxisX"))
    y = _number(ramp.extra.get("planRampRunAxisY"))
...
def _plan_path_choice(path, first, second, obstacles, space, area, footprints, turning_cache, width_cache, path_area):
    measurements = _route_measurements(first, second, path, obstacles, space)
    ...
    for ifc_type in ["IfcSpace", "IfcDoor", "IfcWall", "IfcColumn", "IfcStair", "IfcStairFlight", "IfcRamp", "IfcRampFlight"]:
        for obj in model.by_type(ifc_type):
            ...
            try:
                ...
                pass
            footprint = _shape_footprint(shape) if shape is not None else None
            if footprint is not None and not footprint.is_empty:
                footprints[guid] = footprint
            element = by_guid.get(guid)
            if element and element.ifc_type == "IfcDoor":
                geometry, opening_shape = _plan_door_geometry(obj, shape, settings)
                element.extra.update(geometry)
                fallback_shape = shape if shape is not None else opening_shape
                if fallback_shape is not None:
                    if guid not in footprints:
                        footprint = _shape_footprint(fallback_shape)
                        if footprint is not None and not footprint.is_empty:
                            footprints[guid] = footprint
                    if element.center is None:
                        _set_plan_element_geometry(element, fallback_shape, "IfcOpeningElement geometry" if shape is None else element.source)
                if element.extra.get("derivedDoorWidthM") is None:
                    element.extra["derivedDoorWidthM"] = element.extra.get("doorOpeningWidthM") or element.extra.get("doorLocalWidthM")
                if element.extra.get("derivedDoorHeightM") is None:
                    element.extra["derivedDoorHeightM"] = element.extra.get("doorOpeningHeightM") or element.extra.get("doorLocalHeightM")
                if element.center:
                    element.extra["routeDoorCenterPoint"] = ",".join(f"{value:.4f}" for value in element.center)
            elif element and element.ifc_type in {"IfcRamp", "IfcRampFlight"} and shape is not None:
                measurements = _shape_ramp_measurements(shape)
                if measurements:
                    element.extra.update(measurements)
    return footprints


def _set_plan_element_geometry(element: Element, shape, source: str) -> None:
    verts = getattr(shape.geometry, "verts", None)
    if not verts:
        return
    xs = [float(value) for value in verts[0::3]]
    ys = [float(value) for value in verts[1::3]]
    zs = [float(value) for value in verts[2::3]]
    element.bbox_min = min(xs), min(ys), min(zs)
    element.bbox_max = max(xs), max(ys), max(zs)
    element.width = element.bbox_max[0] - element.bbox_min[0]
    element.depth = element.bbox_max[1] - element.bbox_min[1]
    element.height = element.bbox_max[2] - element.bbox_min[2]
    element.center = tuple((element.bbox_min[index] + element.bbox_max[index]) / 2 for index in range(3))
    element.source = source


def _plan_door_geometry(obj, shape, settings) -> tuple[dict[str, float | str | bool | None], object | None]:
    axes = _placement_axes(obj)
    ...
    if axes:
        ...
        result["doorAxisSource"] = "IfcDoor.ObjectPlacement"
    if body_size:
        result["doorLocalWidthM"] = body_size[0]
        result["doorLocalDepthM"] = body_size[1]
        result["doorLocalHeightM"] = body_size[2]
    if opening_size:
        result["doorOpeningWidthM"] = opening_size[0]
        result["doorOpeningDepthM"] = opening_size[1]
        result["doorOpeningHeightM"] = opening_size[2]
    if opening is not None:
        result["doorOpeningGuid"] = getattr(opening, "GlobalId", None)
    host = _door_host(opening)
    if host is not None:
        result["doorHostGuid"] = getattr(host, "GlobalId", None)
    return result, opening_shape


def _placement_axes(obj):
...
def _shape_local_size(shape, axes) -> tuple[float, float, float] | None:
    verts = getattr(getattr(shape, "geometry", None), "verts", None)
    if not verts:
        return None
    values = [[], [], []]
...
def _shape_ramp_measurements(shape) -> dict[str, float | str] | None:
    ...
    result = {
        "planRampRunLengthM": run,
        "planRampUsableWidthM": width,
        "planRampRiseM": max(0.0, upper[2] - lower[2]),
        "planRampSlopePercent": group["slope"] * 100,
        "planRampRunAxisX": axis[0],
        "planRampRunAxisY": axis[1],
        "planRampWidthAxisX": width_axis[0],
        "planRampWidthAxisY": width_axis[1],
        "planRampMeasurementSource": "IfcRamp sloped surface geometry",
    }
    for prefix, point in [("planRampLowerPoint", lower), ("planRampUpperPoint", upper)]:
        for suffix, value in zip(["X", "Y", "Z"], point):
            result[f"{prefix}{suffix}"] = value
    return result


def _storey_elevations(model) -> list[tuple[str, float]]:
    from ifcopenshell.util.placement import get_local_placement
    ...
    for storey in model.by_type("IfcBuildingStorey"):
        name = str(getattr(storey, "Name", None) or getattr(storey, "GlobalId", "unnamed"))
        elevation = None
        try:
...
def _set_ramp_display_storeys(elements: list[Element], floor_refs: list[tuple[str, float]]) -> None:
    ...
    for ramp in elements:
        if ramp.ifc_type not in {"IfcRamp", "IfcRampFlight"} or not ramp.bbox_min or not ramp.bbox_max:
            continue
        lower_value = ramp.extra.get("planRampLowerPointZ")
        upper_value = ramp.extra.get("planRampUpperPointZ")
...
def build_corridor_width_evidence(
    elements: list[Element],
    prepared: tuple[dict[str, object], dict[str, list[str]], list[dict], list[Element]],
) -> dict[str, dict]:
    footprints, spaces_by_door, _open_boundaries, elements = prepared
    by_guid = {element.guid: element for element in elements}
    doors_by_space: dict[str, list[Element]] = defaultdict(list)
    for door_guid, space_guids in spaces_by_door.items():
        door = by_guid.get(door_guid)
